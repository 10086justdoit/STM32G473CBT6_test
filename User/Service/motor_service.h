...
typedef enum
{
    ...
    MOTOR_SERVICE_ERROR_ALIGN = -6      /**< 传感器对齐错误 */
...
typedef struct
{
    ...
    uint16_t raw_angle;                 /**< 编码器原始角度值，AS5048A 范围 0 ~ 16383 */
    ...
    float ia_lpf;                       /**< A 相电流低通滤波值，单位 A */
    float ib_lpf;                       /**< B 相电流低通滤波值，单位 A */
    float ic_lpf;                       /**< C 相电流低通滤波值，单位 A */
    ...
    float vd;                           /**< D 轴电压指令，单位 V */
    float vq;                           /**< Q 轴电压指令，单位 V */
    ...
    FOC_SVPWM_Data_t svpwm;             /**< SVPWM 中间变量 */

    float zero_electric_angle;          /**< 电角度零点，单位 rad */
    int8_t direction;                   /**< 旋转方向，1 = 正向，-1 = 反向 */
...
Motor_Service_Status_t Motor_Service_Init(void);            /* 初始化电机服务 */
Motor_Service_Status_t Motor_Service_Update(float dt_s);    /* 周期更新电机服务 */
...
void Motor_Service_SetOpenLoopVoltage(float uq);                            /* 设置开环 Uq 电压 */
void Motor_Service_SetOpenLoopFrequency(float frequency_hz);                /* 设置开环电角度频率 */

/* ============================================================
 * 电流环接口
 * ============================================================ */

void Motor_Service_StartCurrentLoop(float id_ref, float iq_ref);            /* 启动电流环模式 */
void Motor_Service_SetCurrentReference(float id_ref, float iq_ref);         /* 设置 D/Q 轴电流指令 */
...
Motor_Service_Status_t Motor_Service_AlignSensor(void);      /* 编码器电角度零位对齐 */

/* ============================================================
 * 状态读取接口
 * ============================================================ */

Motor_Service_Mode_t Motor_Service_GetMode(void);            /* 获取当前模式 */
